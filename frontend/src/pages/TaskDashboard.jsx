...
const TaskDashboard = ({ user, onLogout }) => {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      setIsLoading(true);
      try {
        // CHANGED: Removed the user_id query parameter from the URL
        console.log("Fetching tasks from API..."); // For debugging
        const response = await axios.get(`${API_URL}/tasks`); // CHANGED: Removed the user_id query parameter from the URL (API_URL/tasks);
  ...
  // 2. Add a new task
  const addTask = async () => {
    // CHANGED: Removed the user.id check
    if (newTaskTitle.trim()) {
      try {
        // CHANGED: Removed user_id from the request body
  ...
  const getTaskCardBorder = (status) => {
    switch (status) {
      case "todo": return "border-l-4 border-l-red-500 border-2 border-gray-300";
      case "in-progress": return "border-l-4 border-l-yellow-500 border-2 border-gray-300";
      case "done": return "border-l-4 border-l-green-500 border-2 border-gray-300";
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case "todo": return "text-red-600 bg-red-50";
  ...
  const TaskCard = ({ task }) => (
    <div className={`bg-white ${getTaskCardBorder(task.status)} p-4 mb-4 shadow-sm hover:shadow-md transition-shadow duration-200`}>
      {editingTask === task.id ? (
        ...
        <div>
          ...
          <div className="flex items-center justify-between">
            <select
              ...
              className={`border-2 border-gray-300 rounded-md px-2 py-1 text-sm focus:outline-none focus:border-blue-500 ${getStatusColor(task.status)}`}
            >
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
      ...
      <header className="bg-white border-b-4 border-blue-500 p-4 shadow-sm">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <h1 className="text-2xl font-bold text-gray-800 border-l-4 border-blue-500 pl-3">Task Manager</h1>
      ...
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="bg-white border-2 border-purple-300 border-l-4 border-l-purple-500 p-4 mb-6 rounded-lg shadow-sm">
          <div className="flex gap-4 items-center">
            <div className="flex-1 relative">
              <Search size={20} className="absolute left-3 top-3 text-purple-500" />
              <input
                type="text"
                value={searchTerm}
        ...
        {/* Task Columns */}
        {isLoading ? (
            <div className="text-center text-gray-500 text-xl py-10">Loading tasks...</div>
        ) : (
